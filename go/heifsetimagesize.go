...
func main() {
	var err error
	if len(os.Args) < 5 {
		fmt.Println("Usage: heifsetispe <input HEIF> <width> <height> <output GIF>")
		return
	}
	infile := os.Args[1]
	...
	err = HEIFSetImageSize(bytes, uint32(width), uint32(height))
	if err != nil {
		fmt.Print(err)
		return
	}
	ioutil.WriteFile(outfile, bytes, 0755)
}
